...
exact_match = evaluate.load("/home/lzc/evaluate/metrics/exact_match/exact_match.py")

def _tokenize_and_seg(text, tokenizer, repeat_every_k_tokens=32):
    tokens = tokenizer(text)['input_ids']
    seg_token = tokenizer('<seg>')['input_ids']
    new_tokens = []
    for i, token in enumerate(tokens):
        new_tokens.append(token)
        if (i + 1) % repeat_every_k_tokens == 0:
            new_tokens.extend(seg_token)
    # print(new_tokens)
...
def main(args):
    ...
    with open(os.path.join(args.data_dir, "examplar.jsonl"), "r") as fin:
        GSM_EXAMPLARS = [json.loads(line) for line in fin]
    ...
    if args.model_name_or_path:
        ...
        if args.use_vllm:
            ...
            if args.seg:
                prompts = [_tokenize_and_seg(prompt, tokenizer, repeat_every_k_tokens=args.k) for prompt in prompts]
                print(prompts[0])
    ...
    for output in outputs:
        ...
        output = re.sub('seg|<seg>|114|and|<and>|####', '', output)
