...
def prepare_data(data_name, args):
    ...
    if args.seg != -1:
        ...
        out_file_prefix += f"_{segmentater[args.seg]}"
...
def main(llm, tokenizer, data_name, args):
    ...
    if args.pause:
        for i, prompt in enumerate(input_prompts):
            ...
            if idx != -1:
                input_prompts[i] = prompt[:idx] + '<seg>' * 10 + prompt[idx:]
            else:
                input_prompts[i] = prompt + '<seg>' * 10
    ...
    for epoch in range(max_func_call):
        ...
        for (i, query), output in zip(current_prompts, outputs):
            output = re.sub(r'\<seg\>|seg|\<and\>|and|114|\.\$\?|####', '', output)
