...
class TrainableEmbedding(nn.Module):
    def __init__(self, original_embed, num_add_token):
        super().__init__()
        self.total_vocab_size = original_embed.weight.shape[0]
        self.original_vocab_size = original_embed.weight.shape[0] - num_add_token
        self.num_new_tokens = num_add_token
        self.embed_dim = original_embed.weight.shape[1]
        
        # 冻结原始embedding参数
        self.original_weight = nn.Parameter(
            original_embed.weight[:self.original_vocab_size].detach(),
            requires_grad=False
        )
        ...
        # 保持其他embedding属性一致
        self.padding_idx = original_embed.padding_idx
        self.max_norm = original_embed.max_norm
        self.norm_type = original_embed.norm_type
        self.scale_grad_by_freq = original_embed.scale_grad_by_freq
        self.sparse = original_embed.sparse

    def forward(self, input_ids):
        combined_weights = torch.cat([self.original_weight, self.new_weight], dim=0)
        return F.embedding(
            input_ids,
            combined_weights,
            self.padding_idx,
...
class TrainableLMHead(nn.Module):
    def __init__(self, original_head, num_add_token):
        super().__init__()
        self.total_vocab_size = original_head.weight.shape[0]
        self.original_vocab_size = original_head.weight.shape[0] - num_add_token
        self.num_new_tokens = num_add_token
        self.embed_dim = original_head.weight.shape[1]
        # 冻结原始head参数
        self.original_weight = nn.Parameter(
            original_head.weight[:self.original_vocab_size].detach(),
            requires_grad=False
        )
    ...
    def forward(self, x):
        combined_weights = torch.cat([self.original_weight, self.new_weight], dim=0)
        return F.linear(x, combined_weights, None)
    ...
    def merge(self):
        ...
        merged_linear.weight = nn.Parameter(combined_weights)
        return merged_linear

def replace_embeddings_and_head(model, num_add_token):
    # 替换embedding层
    original_embed = model.base_model.model.model.embed_tokens
    model.base_model.model.model.embed_tokens = TrainableEmbedding(original_embed, num_add_token)
    
    # 替换lm_head层
    original_head = model.base_model.model.lm_head
    model.base_model.model.lm_head = TrainableLMHead(original_head, num_add_token)
    
    return model

def merge_embeddings_and_head(model):
    replaced_embed : TrainableEmbedding = model.base_model.model.model.embed_tokens
    model.base_model.model.model.embed_tokens = replaced_embed.merge()
    
    replaced_head : TrainableLMHead = model.base_model.model.lm_head
    model.base_model.model.lm_head = replaced_head.merge()
...
def main(args: FlatArguments):
    ...
    if args.use_lora:
        ...
        peft_config = LoraConfig(
            ...
            target_modules=[
                ...
                "up_proj", 
                "down_proj"
            ] if not args.qv_only else ["q_proj", 'v_proj']
        )
        model = get_peft_model(model, peft_config)
        replace_embedding = args.ft_part_embed
        if replace_embedding:
            model = replace_embeddings_and_head(model, num_added_tokens)
        else:
    ...
    with accelerator.main_process_first():
        train_dataset = train_dataset.map(
            ...
            cache_file_name='cache/downsample_tulu3_seg/.downsample_tulu3_seg'
        )
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if args.logging_steps and completed_steps % args.logging_steps == 0:
                    ...
                    # check if avg_loss is nan
                    if math.isnan(avg_loss):
                        raise Exception("avg_loss is None")
    ...
    if args.use_lora:
        if replace_embedding:
            model = merge_embeddings_and_head(model)
    ...
    if accelerator.is_local_main_process:
        clean_last_n_checkpoints(args.output_dir, keep_last_n_checkpoints=2)
